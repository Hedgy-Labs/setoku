...
describe("panel row cap decoupled from run_query (byte-bounded render)", () => {
  ...
  const mkRowsDash = (id: string, sql: string) => ({
    ...
    createdAt: new Date().toISOString(), archivedAt: null, lockedAt: null, lockedBy: null, hasPassword: false,
    panels: [{ key: "kpi", title: "n", sql, dialect: "clickhouse" as const }],
    params: [],
  });
  ...
  it("splits the byte budget fairly across panels — none is dropped whole", async () => {
    ...
    const dash = {
      ...
      refreshSeconds: 30, visibility: "team" as const, createdBy: "test", createdAt: new Date().toISOString(), archivedAt: null, lockedAt: null, lockedBy: null, hasPassword: false,
