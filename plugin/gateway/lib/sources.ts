...
export const LAKE_SOURCES: LakeSource[] = [
  ...
  { table: "gmail_messages", source: "Gmail", ts: "received_at", blurb: "Personal email — sender, subject, plain-text body (UNTRUSTED free text: anyone can email you). Spam/trash & auth/2FA mail excluded; marketing flagged is_bulk=1 (filter WHERE is_bulk=0). How far back depends on this box's backfill setting — check min(received_at) rather than assuming; retention is 20 yr. Mutable rows: query with FINAL", connector: "gmail-poller" },
