...
**`ingest/`** — containers that fill the lake: `slack-listener`, `mercury-poller`, `render-poller`, `github-poller`, `pg-mirror` (mirror of the business DB into ClickHouse `biz.*` — full reloads, or `xmin`-incremental upserts into a ReplacingMergeTree where a table qualifies; the default read path for heavy app panels; re-derivable, so excluded from backups per I4), plus the lake's SQL schemas in `ingest/schemas/`. Liveness is tracked via `ingest_heartbeats` (a "flowing" source reads from a real beat, not data recency).
