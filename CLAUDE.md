...
- **Gateway chrome is neutral (stone), never branded.** The admin SPA and gateway-served surfaces (the `/admin` app, the public dashboard shell) use a monochrome **stone** palette and introduce **no accent color of their own**. Color belongs to the user's content — their dashboards and charts. Defer to how the user's dashboards look: chrome (buttons, badges, links, toasts) stays stone-family so it never competes with or clashes against whatever a published dashboard renders. When you reach for a teal/blue/etc. accent on a gateway element, that's the smell — make it stone.
