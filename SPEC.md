...
- v0.11 — **live dashboards supersede static Reports.** A report was frozen HTML (data photographed at publish time); a dashboard splits **presentation** (frozen, agent-authored template) from **data** (named **panels**, each a saved `sql`+`dialect` the box re-runs through the governed `run_query` path, TTL-cached, audited as `dashboard:<id>`). Rendered as **trusted outer shell + sandboxed inner frame**: data is *injected* not fetched, so the frame runs under `default-src 'none'` (no network) — closing the exfil-via-author-JS hole static reports had. Tools `publish_report`/`list_published`/`unpublish_report` → `publish_dashboard` (dry-runs + cache-seeds panels at publish), `list_dashboards`, `unpublish_dashboard`, `get_dashboard`. **Inspection** ("how is this calculated"): a provenance drawer in the shell shows SQL (team-only) / metric definition + as-of; public `/p/<id>/data` omits raw SQL. Public promotion stays a human `/admin` click (membrane intact — a dashboard is a publish artifact, never a curated-knowledge write). Storage extends `published` in place (`panels`, `refresh_seconds`) + a `dashboard_cache` table; legacy `format='html'` reports keep rendering. Out of v1: viewer-supplied filter params (need bound/typed params — v2) and a pre-warm refresh cron. (docs/dashboards.md.)
